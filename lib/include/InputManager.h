...
//DirectInput（XBOXではないUSBコントローラ）設定。ただし、WindowsStoreやXBOXではこっちが動かない。
//正直、対応しないソフトも出てくると思うので、今はxInput対応って書いてあるパッドを買う方が安全。120fポーリングも対応してくるだろうし。
//DirectInputはかなり古い（最終版がDirectX8）APIで、非推奨だけどもSteamで売るなら対応していた方が良い。そろそろなくて良いかも？
//ただ、xInputも結構仕様が古くて、いい加減Updateしてもらわないと困る。少なくともインパルストリガーには対応せぇよと。
//インパルストリガーとシイタケボタンもDLL直読みで使う事は出来るっちゃ出来るんだけども・・・
#define DIRECT_INPUT_ACTIVE 1	//もうゲーム機とかだと無くて良いしね。
...
#define MAX_DI_PADS 4			//とりあえず４つにしておくけども、DirectInputは32個まで接続出来るぞ。
#define MAX_DI_BUTTONS 32		//とりあえず32個にするけども、DirectInputのボタンは最大128だ。鉄騎コンも動かせるぞ（要改造・要ドライバ自作）
#define MAX_DI_ANALOGIN 32		//POVとrgbButtons以外に後３２箇所のデータがあるんだけどもどれがどれだかは決まっていないのでハードごとにバラバラ。
#endif


#define MAX_KEYS 256
#define MAX_MOUSE_BUTTONS 5
#define MAX_PAD_BUTTONS 16		//上下左右＋１０ボタンまで対応・・・だけど実は使われていない2bitが・・・
...
public:
	...
	enum class X_BUTTONS
	{
		...
		BTN_GUIDE,			//XBOX ガイドボタン	正規のxInputだと取れない。DLLから関数ポインタ引っ張ると取れる。
		BTN_UNKNOWN,		//多分未使用
