...
//ComPtr<>を使うと確かにリスクは減るんだけど、書き間違いのリスクは高くなるので注意。
//何かと言うと&で参照とってもコンパイルエラーにならないんだけど例外が出たり誤動作する。
//参照は必ずGetAddressOf()メソッドで。
...
private:
    ...
    //ComPtr<ID3D12Resource> m_pSamplerLinear = nullptr;
    ...
    ////描画用ゲームオブジェクト
    //std::list<GameObject*>              m_drawObjects;     //ここに入っている物を描画する
...
public:
    ...
    //=========DX12 Synchronization objects
    void WaitForGpu();
    void WaitForGpu(int frame); //フレーム番号指定
    ...
    // DSVヒープの開始CPUディスクリプタハンドルを返す
    D3D12_CPU_DESCRIPTOR_HANDLE GetDSVHeapStartHandle()
    {
        return m_dsvHeap->GetCPUDescriptorHandleForHeapStart();
    }

    //void AddDrawObject(GameObject* obj)                     //GameObjectを描画リストに追加。これは毎フレーム行う。
    //{
    ...
    void UpdateCameraMatrixForComponent(float fov, XMVECTOR Eye, XMVECTOR At, XMVECTOR Up, float width, float height, float nearZ, float farZ, const XMFLOAT4& viewport);
