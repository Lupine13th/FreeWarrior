...
bool isHitVS(HitOBB* atk, HitOBB* def, XMFLOAT3& resHitPos);			//OBB VS OBB	*必須
...
bool isHitVS(HitOBB* atk, HitRayLine* def, XMFLOAT3& resHitPos, bool revCheck);		//OBB VS 線分	*必須
...
bool isHitVS(HitSphere* atk, HitRayLine* def, XMFLOAT3& resHitPos, bool revCheck);	//球 VS 線分	*必須

bool isHitVS(HitPillar* atk, HitPillar* def, XMFLOAT3& resHitPos);	//円柱 VS 円柱	*必須
...
{
	XMFLOAT4 myRect = atk->getHitRect();			//このHitStructureが持つ判定領域を取得
...
bool getVectorT(const float max, const float min, const float nml, const float pos, float& sT, float& eT)
{
	...
	//交差開始点０未満はベクトルの逆方向になる
	if (sT < 0.0f) sT = 0.0f;

	//eTがマイナスなのはベクトルが完全に反対方向
