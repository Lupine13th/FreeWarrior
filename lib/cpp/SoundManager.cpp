...
//0:反乱軍BGM 1:帝国軍BGM 2:カーソル移動音 3:決定ボタン音 4:準備完了SE 5:準備完了決定SE 6:キャンセル音
...
{
	if (m_playingContainers[0])
	{
		m_playingContainers[0]->update();
		if (m_CurrentBgmId != -1 && m_playingContainers[0]->isFinished())
		{
			// BGMを再再生
			m_playingContainers[0]->playWavData(m_pXaudio2, m_soundContainers[m_CurrentBgmId]->getWavData());
		}
	}
...
{
	//コンテナを停止・解放
	for (auto& se : m_playingContainers)
	{
		if (se)
		{
			se->stop();
...
void SoundManager::playBGM(UINT soundId)
{
	stopBGM(); // 既存のBGMを止める

	if (SUCCEEDED(m_playingContainers[0]->playWavData(m_pXaudio2, m_soundContainers[soundId]->getWavData())))
	{
		m_CurrentBgmId = soundId;	// 再生に成功したらIDを保存
	}
	else
	{
		m_CurrentBgmId = -1; // 再生に失敗したらIDをリセット
	}
}

void SoundManager::stopBGM()
{
	m_playingContainers[0]->stop();
	m_CurrentBgmId = -1;
}
