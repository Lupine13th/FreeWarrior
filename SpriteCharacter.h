...
private:
	...
	//ComPtr<ID3D11Buffer> m_pVertexBuffer = nullptr;	//UVやサイズが変わるのでVertexBufferはここで持ってしまう。

	const wchar_t*	index_id = L"Sprite";			//Vertex, IndexバッファID。これはプリセットなので固定で行きます。

	const wchar_t*	texture_id = nullptr;			//テクスチャID（複数のテクスチャをIDで選択できるようにしている）
	std::vector<std::unique_ptr<SpritePattern>>		m_uvArray;	//スプライトパターン、つまりUV値のリスト
