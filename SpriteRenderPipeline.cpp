...
HRESULT SpriteRenderPipeline::InitPipeLineStateObject(ID3D12Device2* d3dDev)
{
    ...
    // バージョンテスト。RootSignatureはVer1.1があるんだけども、アップデートしていないと環境が対応しない。その場合は1.0で作成する
...
ID3D12GraphicsCommandList* SpriteRenderPipeline::ExecuteRender()
{
    ...
    //256bytes allignment
    //今回は結局全部のサイズが256なんだけども、どんなサイズのデータでも大丈夫なように、の基本構造。
    ...
    for (auto charaData : m_renderList) //renderList登録が出来ていない
    {
        ...
        for (const auto& currentLabel : cameralabelList)
        {
            ...
            {
                D3D12_VIEWPORT viewport = { 0.0f, 0.0f, (FLOAT)myEngine->GetWidth(), (FLOAT)myEngine->GetHeight(), 0.5f, 1.0f };
                cmdList->RSSetViewports(1, &viewport);
                D3D12_RECT scissorRect = { 0, 0, (LONG)myEngine->GetWidth(), (LONG)myEngine->GetHeight() };
                cmdList->RSSetScissorRects(1, &scissorRect);
            }
